...
let[@inline] blit_bounds_check_exn t len =
  ...
  if len < 0 then raise (Exception (BlitNegativeLength len));
  let last = offset + len - 1 in
  if offset < 0 || (len > 0 && (last < 0 || last >= t.len))
  then raise (Exception (BlitOutOfBounds (offset, len, t.len)));
  offset
;;
...
let realloc_exn t new_index =
  ...
  in
  if new_len < needed
  then raise (Exception (MaxAllocationReached (new_index, t.max_size)));
...
let create ?(max_size = 32000) initial_size =
